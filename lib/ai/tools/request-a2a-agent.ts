      ...
      try {
        ...
        const canvasContent =
          createdTasks.length > 0
            ? `# ${title || 'Task Canvas'}\n\n✅ Successfully created ${createdTasks.length} tasks for execution:\n\n${createdTasks.map((t) => `- ${(t.result as any)?.title || 'Task'}`).join('\n')}\n\nTasks are ready for execution.`
        ...
        if (createdTasks.length > 0) {
          ...
          console.log('[A2A Tool] ✅ Linked stored tasks to canvas:', {
            documentId: canvasDocument.id,
            taskIds,
        ...
        return {
          ...
          content:
            createdTasks.length > 0
              ? `✅ Successfully created and stored ${createdTasks.length} tasks in database. Canvas document created with proper task linkage.`
